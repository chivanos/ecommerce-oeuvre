...
export default function UpdateMainRoy(){    
    const [content,setContent] = useState(['']);
    let params = useParams();
    let url=`http://localhost:8000/api/list/roy/${params.id}`;

    useEffect(()=> {
        const fetchAll = () => {
            axios.get(`http://localhost:8000/api/list/roy/${params.id}`)
    ...
    return (
        <>
            <div className="col-md-12">
                <Manage
                    header={<HeaderSecond title={"Update Royal tresuares"} />}
                    ...
                    titre={"Update"}
                    composant={<Filling dashboard={<UpdateFilling content={content} url={`http://localhost:8000/api/list/roy/`}/>}/>}
