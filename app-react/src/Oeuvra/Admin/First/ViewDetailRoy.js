...
export default function ViewDetailsRoy(props){    
    const [content,setContent] = useState([""]);
    let params = useParams();
    let url=`http://localhost:8000/api/details/roy/${params.id}`;

    useEffect(()=> {
        const fetchAll = () => {
            axios.get(`http://localhost:8000/api/details/roy/${params.id}`)
    ...
    return (
        <>
            <div className="col-md-12">
                <Manage 
                    header={<HeaderSecond title={"Tresuares Royal Details"} />}
                    ...
                    titre={"Details"}
                    composant={<Filling dashboard={<FillingView contain={content} params={params.id} url={"roy"}  />}/>}
