...
export default function ViewDetailsCont(props){    
    const [content,setContent] = useState([]);
    let params = useParams();
    let url=`http://localhost:8000/api/details/cont/${params.id}`;

    useEffect(()=> {
        const fetchAll = async() => {
            await fetch(`http://localhost:8000/api/details/cont/${params.id}`)
    ...
    return (
        <>
            <div className="col-md-12">
                <Manage 
                    header={<HeaderSecond title={"Contemporary Details"} />}
                    ...
                    titre={"Details"}
                    composant={<Filling dashboard={<FillingView contain={content} params={params.id} url={"cont"}  />}/>}
